        ...
        if self.format_as_list:
            if self.indent is None:
                ...
                if self.separators == (",", ":"):
                    ...
                    # Use tojson as-is (spaced format)
                    return "{{ tools | tojson }}"
            else:
...
class JsonFormatterNoBreakLine(JsonFormatter):
    """
    JSON formatter for Qwen models.
    """
...
class KimiK2ToolCallContentProcessor(ToolContentProcessor):
    ...
    def __call__(self, tool: Dict) -> Dict:
        ...
        return f"{id}<|tool_call_argument_begin|>{arguments}"
    ...
    def jinja(self) -> str:
        ...
        """
        return (
            "{% set tool_id = tool['id'] if 'id' in tool else '' %}"
            "{% set tool_call = tool['function'] if 'type' in tool and tool['type'] == 'function' else tool %}"
