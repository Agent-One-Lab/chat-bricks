...
class Renderer:
    ...
    def _render_tool_calls(self, tool_calls: List[Dict]) -> str:
        ...
        for tool_call in tool_calls:
            if self.template.tool_policy.tool_call_content_processor is not None:
                tool_call_str = self.template.tool_policy.tool_call_content_processor(tool_call)
            else:
                if 'type' in tool_call and tool_call['type'] == 'function':
                    tool_call = tool_call['function']
                tool_call_str = json.dumps(tool_call)
            full_tool_calls_str.append(self.template.tool_call_template.format(tool_call=tool_call_str))
        
        if self.template.tool_calls_template is not None:
            return self.template.tool_calls_template.format(tool_calls="".join(full_tool_calls_str))
        else:
        ...
        if isinstance(tool_observation_content, str):
            ...
            for item in tool_observation_content:
                if item["type"] == "text":
                    ...
                    text += self.template.vision_start + self.template.image_token + self.template.vision_end
    ...
    def _encode_system_message_default(self, tools=None) -> str:
        logger.debug(f"[Template] Encoding system message default for template: {self.template.name}")
        ...
        if self.template.system_policy.content_processor is not None:
            system_message = self.template.system_policy.content_processor(self.template.system_message, tools=tools)
        else:
        ...
        if tools is None:
            ...
            if self.template.system_template_with_tools:
                return self.template.system_template_with_tools.format(system_message=system_message, tools=tools)
            else:
                return self.template.system_template.format(system_message=system_message)

    def _encode_system_message(self, content, tools=None) -> str:
        # Handle both string content and list content formats
        logger.debug(f"[Template] Encoding system message for template: {self.template.name}")
        ...
        if self.template.system_policy.content_processor is not None:
            system_message = self.template.system_policy.content_processor(system_message, tools=tools)
        ...
        if isinstance(content, str):
            ...
            for item in content:
                if item["type"] == "text":
                    ...
                    text += self.template.vision_start + self.template.video_token + self.template.vision_end
                else:
                    raise ValueError(f"Invalid message type: {item['type']}")
        
        if self.template.user_template_with_tools:
            user_message = self.template.user_template_with_tools.format(content=text, tools=tools)
        else:
    ...
    def _encode_assistant_message(self, content, tool_calls_str=None) -> str:
        ...
        if "{tool_calls}" in self.template.assistant_template:
            assistant_message = self.template.assistant_template.format(content=text, tool_calls=tool_calls_str if tool_calls_str else "")
        else:
    ...
    def _encode_generation_prompt(self) -> str:
        ...
        if "{content}" in self.template.assistant_template:
            ...
            raise ValueError(f"Assistant template {self.template.assistant_template} does not contain {{content}}")
        ...
        if not has_existing_think:
            ...
            for i in range(len(elements) - 1, -1, -1):
                if roles[i] == Role.ASSISTANT:
                    ...
                    elements[i] = generation_prefix + "<think>\n\n</think>\n\n" + elements[i][len(generation_prefix):]
                    break
...
class HFRenderer(Renderer):
    ...
    def _render_message_turn(
        ...
        **kwargs
        ...
        current_prompt = self.template.tokenizer.apply_chat_template(
            current_messages, tokenize=False, tools=current_tools, add_generation_prompt=False, **kwargs
        )
    ...
    def _add_generation_prompt(self, messages: List[Dict], turn_idx: int, current_prompt: str,
                               tools=None, **kwargs) -> Tuple[str, str]:
        ...
        gen_prompt = self.template.tokenizer.apply_chat_template(
            current_messages, tokenize=False, tools=current_tools, add_generation_prompt=True, **kwargs
        )
    ...
    def render(self, messages: List[Dict], tools=None, add_generation_prompt: bool = False, **kwargs) -> Tuple[str, List[str], List[bool]]:
        ...
        if not messages:
            prompt = self.template.tokenizer.apply_chat_template(
                [], tokenize=False, tools=tools, add_generation_prompt=add_generation_prompt, **kwargs
            )
            return prompt, [prompt], [True]  # Generation prompt or empty prompt is masked
        ...
        for i in sorted(render_indices):
            ...
            if new_element:
                ...
                mask_flags.append(not turn_is_assistant)  # True for non-assistant, False for assistant
        ...
        if add_generation_prompt:
            gen_prompt = self.template.tokenizer.apply_chat_template(
                messages, tokenize=False, tools=tools, add_generation_prompt=True, **kwargs
            )
            ...
            final_prompt = self.template.tokenizer.apply_chat_template(
                messages, tokenize=False, tools=tools, add_generation_prompt=False, **kwargs
            )
