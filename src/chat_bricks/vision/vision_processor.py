...
class VisionProcessor(ABC):
    ...
    def preprocess_images(
        self, images: List["ImageInput"], processor: Any
    ) -> Dict[str, Any]:
    ...
    def preprocess_videos(
        self, videos: List["VideoInput"], processor: Any
    ) -> Dict[str, Any]:
    ...
    def process_for_llm(
        ...
        for element, mask_flag in zip(elements, mask_flags):
            ...
            if self._contains_vision_tokens(element):
                ...
                expanded_element = self.expand_vision_tokens(
                    element, images_to_process, videos_to_process, processor
                )
                cur_input_ids = tokenizer.encode(
                    expanded_element, add_special_tokens=False
                )
            else:
        ...
        assert len(images_to_process) == len(videos_to_process) == 0, (
            f"All images and videos should be processed, but got {len(images_to_process)} images and {len(videos_to_process)} videos left for vision template {self.config.model_type}."
        )
...
class PatchBasedProcessor(VisionProcessor):
    ...
    def _load_image_from_input(self, image_input) -> "ImageObject":
        ...
        if isinstance(image_input, str):
            ...
            elif image_input.startswith("data:image/") or image_input.startswith(
    ...
    def _preprocess_single_image(self, image: "ImageObject", **kwargs) -> "ImageObject":
        ...
        if (image.width * image.height) > self.config.image_max_pixels:
            resize_factor = math.sqrt(
                self.config.image_max_pixels / (image.width * image.height)
            )
            width, height = (
        ...
        if (image.width * image.height) < self.config.image_min_pixels:
            resize_factor = math.sqrt(
                self.config.image_min_pixels / (image.width * image.height)
            )
            width, height = (
                int(image.width * resize_factor),
                int(image.height * resize_factor),
    ...
    def _regularize_videos(
        ...
        for video in videos:
            ...
            if isinstance(video, list) and all(
                isinstance(frame, (str, BinaryIO, dict)) for frame in video
            ):
                ...
                if total_frames == 0:  # infinite video
                    sample_indices = np.linspace(
                        0, self.config.video_maxlen - 1, self.config.video_maxlen
                    ).astype(np.int32)
                else:
                    sample_frames = max(
                        1,
                    ...
                    sample_frames = min(
                        total_frames, self.config.video_maxlen, sample_frames
                    )
                    sample_indices = np.linspace(
                        0, total_frames - 1, sample_frames
                    ).astype(np.int32)
    ...
    def calculate_image_tokens(self, image_data: Dict[str, Any], processor: Any) -> int:
        ...
        if "pixel_values" in image_data:
            ...
            if (
                ...
                and processor.vision_feature_select_strategy == "default"
            ):
                image_seqlen -= 1
            return image_seqlen
        return 1
    ...
    def expand_vision_tokens(
        ...
        processed_images = [
            self.preprocess_images([image], processor) for image in images_slice
        ]
        processed_videos = [
            self.preprocess_videos([video], processor) for video in videos_slice
        ]
            ...
            for idx in range(len(parts) - 1):
                if idx < len(processed_videos):
                    ...
                    if "pixel_values" in processed_video:
                        video_tokens = self.calculate_video_tokens(
                            processed_video, processor
                        )
...
class QwenVLProcessor(PatchBasedProcessor):
    ...
    def expand_vision_tokens(
        ...
        expanded_prompt = super().expand_vision_tokens(
            prompt, images, videos, processor
        )
...
def register_processor(template_name: str, config: VisionProcessorConfig):
    ...
    if processor_class is None:
        raise ValueError(
            f"No processor class found for model type: {config.model_type}"
        )
    VISION_PROCESSORS[template_name] = processor_class(config)


def register(
    cls, template_name: str, config: VisionProcessorConfig, processor_class: type = None
):
    """Register a vision processor for a template"""
